import React from 'react'

const SearchIcon = () => {
  return (
    <svg width="31" height="31" viewBox="0 0 31 31" fill="none" xmlns="http://www.w3.org/2000/svg">
      <g clipPath="url(#clip0_8_93)">
        <rect width="31" height="31" fill="white" fillOpacity="0.21" />
        <path d="M20.0208 18.0833H19.0004L18.6388 17.7346C19.9046 16.2621 20.6667 14.3504 20.6667 12.2708C20.6667 7.63375 16.9079 3.875 12.2708 3.875C7.63375 3.875 3.875 7.63375 3.875 12.2708C3.875 16.9079 7.63375 20.6667 12.2708 20.6667C14.3504 20.6667 16.2621 19.9046 17.7346 18.6388L18.0833 19.0004V20.0208L24.5417 26.4662L26.4662 24.5417L20.0208 18.0833ZM12.2708 18.0833C9.05458 18.0833 6.45833 15.4871 6.45833 12.2708C6.45833 9.05458 9.05458 6.45833 12.2708 6.45833C15.4871 6.45833 18.0833 9.05458 18.0833 12.2708C18.0833 15.4871 15.4871 18.0833 12.2708 18.0833Z" fill="black" fillOpacity="0.35" />
      </g>
      <defs>
        <clipPath id="clip0_8_93">
          <rect width="31" height="31" fill="white" />
        </clipPath>
      </defs>
    </svg>

  )
}

export default SearchIcon
